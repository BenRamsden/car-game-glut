...
void Helpers::DrawTexturedCone(GLfloat top_radius, GLfloat bottom_radius, GLfloat height, GLint texId)
{
	...
	for (GLfloat angle = 0.0f; angle < target_angle; angle += step) {
		...
		glNormal3f(cos(angle), sin(angle), 0.0f);
		glTexCoord2f(0.0f, angle/target_angle);
		glVertex3f(top_x, top_y, height);
		glTexCoord2f(height/50, angle / target_angle);
		glVertex3f(bottom_x, bottom_y, 0.0f);
	}
